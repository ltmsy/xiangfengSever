...
func (a *adminConfigDB) GetConfigsByCategory() ([]*ConfigCategory, error) {
	...
	_, err := a.session.Select("*").From("admin_system_config").Where("is_deleted = 0").GroupBy("category").OrderBy("MIN(sort_order)").Load(&categories)
	if err != nil {
...
func (a *adminConfigDB) GetConfigsByCategoryKey(category string) ([]*AdminSystemConfig, error) {
	...
	_, err := a.session.Select("*").From("admin_system_config").Where("category = ? AND is_deleted = 0", category).OrderBy("sort_order ASC, id ASC").Load(&configs)
	if err != nil {
...
func (a *adminConfigDB) GetConfigByKey(configKey string) (*AdminSystemConfig, error) {
	...
	_, err := a.session.Select("*").From("admin_system_config").Where("config_key = ? AND is_deleted = 0", configKey).Load(&config)
	if err != nil {
...
func (a *adminConfigDB) UpdateConfig(configKey, configValue, updatedBy string) error {
	result, err := a.session.Update("admin_system_config").SetMap(map[string]interface{}{
		...
		"updated_at":   "NOW()",
		"version":      "version + 1",
	}).Where("config_key = ? AND is_deleted = 0", configKey).Exec()
...
func (a *adminConfigDB) GetPublicConfigs() (map[string]string, error) {
	var configs []*AdminSystemConfig
	_, err := a.session.Select("config_key, config_value").From("admin_system_config").Where("is_public = 1 AND is_deleted = 0").Load(&configs)
	if err != nil {
...
func (a *adminConfigDB) DeleteConfig(configKey string) error {
	_, err := a.session.Update("admin_system_config").Set("is_deleted", 1).Where("config_key = ?", configKey).Exec()
